...
def yaml_to_markdown(yaml_file, markdown_file):
    try:
        ...
        print(f"Error parsing YAML file {yaml_file}: {exc}")
        return

    if not isinstance(data, dict):
        print(f"Error: Parsed YAML data is not a dictionary in file {yaml_file}")
        return
    ...
    try:
                ...
                if 'choose' in action:
                    for choice in action['choose']:
                        ...
                        for step in choice.get('sequence', []):
                            ...
                            step_data = step.get('data', 'No data')
                            f.write(f"- **{service}**: {step_data}\n")
                else:
                    if 'service' in action:
                        service = action.get('service', 'Unknown service')
                        action_data = action.get('data', 'No data')
                        f.write(f"- **{service}**: {action_data}\n")
                    if 'delay' in action:
        ...
        print(f"Error writing to Markdown file {markdown_file}: {exc}")
